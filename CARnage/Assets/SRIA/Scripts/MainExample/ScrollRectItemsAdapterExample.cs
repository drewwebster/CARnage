...
namespace frame8.ScrollRectItemsAdapter.MainExample
{
    ...
    public class ScrollRectItemsAdapterExample : SRIA<MyParams, ClientItemViewsHolder>, ExpandCollapseOnClick.ISizeChangesHandler
	{
		...
		MOD_Model CreateNewModel(int index)
        {
            GameObject[] modArray = Resources.LoadAll<GameObject>("MODSResources");
            CARnageModifier mod = modArray[index].GetComponent<CARnageModifier>();
            var model = new MOD_Model()
            {
                ...
                //clientName = "Detonation",//_Params.sampleFirstNames[Rand(_Params.sampleFirstNames.Length)],
				//clientName = "Client #" + index,
				//location = "Create a huge explosion when you are destroyed.", //_Params.sampleLocations[Rand(_Params.sampleLocations.Length)],
	...
	public class ClientItemViewsHolder : BaseItemViewsHolder
	{
		...
		public void UpdateViews(MyParams p)
		{
            ...
            avatarImage.sprite = dataModel.modImage;// p.sampleAvatars[dataModel.avatarImageId];
            nameText.text = dataModel.modName;// + "(#" + ItemIndex + ")";
			locationText.text = "  " + dataModel.description;
		...
		void UpdateScores(MOD_Model dataModel)
		{
		...
		void UpdateFriendsAvatars(MOD_Model dataModel, MyParams p)
		{
            return;
	...
	[Serializable]
	public class MyParams : BaseParamsWithPrefabAndLazyData<MOD_Model>
	{
